---
layout: page
title: Resume
permalink: /resume/
pane_intro: RISD Sculpture and Interior Architecture alum, Immersive Entertainment Art Director and Artist
resume_pdf_url: /assets/resume/Jaden-Andrea-Resume.pdf
---

## Contact

- Name: Jaden Andrea
- Title: Art Director, Art Team Manager, Sculptor, Creative Technologist
- Location: Portland, OR
- Phone: 714.337.1028
- Email: [nostalgiapurgatory@gmail.com](mailto:nostalgiapurgatory@gmail.com)
- LinkedIn: [linkedin.com/in/jadenandrea](https://www.linkedin.com/in/jadenandrea)
- Portfolio: [jadenandrea.com](https://www.jadenandrea.com/)
- GitHub: [github.com/nostalgiapurgatory](https://github.com/nostalgiapurgatory)

## Professional Summary

Art Director and multidisciplinary artist with 10+ years of experience delivering stylized 3D environments, props, and character/narrative-driven physical environments for immersive entertainment and interactive installations. Strong in translating concept art and reference into build-ready forms through small or large scale models, virtual and physical prototyping, orthographic and other technical drawing, fabrication across disciplines and machine shops, and wholesome cross-department collaboration. Experienced working at unconventional scales, balancing artistic intent with contextual and technical fabrication constraints, production schedules, and clever use of budget.

## Core Competencies

- Imaginary world inspired set and prop development
- Concept-to-3D translation (2D art to physical form)
- Orthographic drafting and build documentation
- Miniature scale, stylized proportion, forced perspective
- Mockup development, rapid prototyping, alternating physical fabrication and 3D printing design process and iterative design reviews
- Vendor handoff prep (CNC, laser cutting, fabrication files)
- Character sculpting, mold/cast workflows, mixed-material fabrication
- Deadline-driven, cross-functional production communication
- Studio safety practices and collaborative team support

## Professional Experience

### [Unannounced Project] -- Los Angeles, CA

**Character Sculptor, Creative Figurine & Product Concepts**  
Dates: May 2026 -- Ongoing  
- Developed 3D character concepts from 2D illustration for an upcoming product series and immersive experience, partnering directly with Creative Director feedback loops.
- Iterated rapidly through mood boards, sketches, and sculpt passes to define final silhouette, proportion, and character readability.

### Bloomsbury Fine Art & Antiques -- Portland, OR

**Painting & Sculpture Restorer & Conservationist**  
Dates: January 2025 -- Ongoing  
- Handled, cleaned, repaired, and restored antique paintings, ornate plaster and compo frames, and sculptures with a mix of materials both historic and contemporary.
- Performed precision restoration with advanced patching, fills, and texture/color matching to make repairs visually imperceptible.
- Highly effective at style matching, from classic and masters' artworks to impressionist and modern art techniques. Is very practiced in blending style to match an existing canvas.
- Proficient in historical material repair, oil paint color matching, and appropriate use of solvents for delicate but effective cleaning and varnish removal.
- Skilled at sculpting completely missing areas of sculptures and frames, where educated guesswork and historical context lead to creating believable form from next to nothing.

### Hi-Orbit Games (Wild Heart Ranch) -- 29 Palms, CA

**Creative Technologist, Engineer, & Fabricator**  
Dates: May 2024 - Nov 2024  
- Designed circuits and Arduino code as well as designed and fabricated creative interactive housing for a number of escape room games and immersive interactive props, puzzles, and set dressing.
- Collaborated with other contracted engineers to create complex projects, where some would design the electronics and send a partially built circuit by mail long distance, to have that circuit rebuilt, reflashed, and housed in a creative and 'to-theme' manner.
- Designed custom PCBs (circuit boards) to control switches, sensors, LEDs, and programmed logic. Conducted playtesting, made design revisions, and ordered those custom designed boards from board shops to make streamlined one-of-a-kind escape room solutions.
- Integrated custom circuits and code to a proprietary show control system used to activate dozens of effects and puzzles for throughput of thousands of guests.
- Prepared fabrication-ready assets and coordinated technical handoff for outsourced and remote build components.
- Integrated iterative prototyping, playtesting, and revision cycles under production deadlines.

### Gather (gather.town) - Fully Remote

**Art Director**  
Dates: Mar 2023 - Dec 2023  
- Supported startup company goals through user interviews, market research, and close partnership with the executive team.
- Developed potential alternative visual styles for future company direction.
- Partnered with Product, Marketing, and Engineering teams on core visual assets and internal tools.
- Guided decisions related to art department contribution to user engagement and retention.

**Art Team Manager (Avatars + Virtual Offices), Internal Promotion**  
Dates: Jun 2021 - Mar 2023  
- Maintained and updated internal dashboard of experimental and standard avatar skins, wearables, object assets, sounds, and mapmaking tools.
- Led a team of artists, virtual office designers, and contractors.
- Defined visual system quality standards for scale, perspective, color palette, diversity, and texture.
- Contributed to localization and cultural research to expand the platform globally.

**Senior Pixel Artist (IC)**  
Dates: Dec 2020 - Jun 2021  
- First full-time artist at Gather.
- Developed visual identity through iterative work on pixel objects, background textures, and virtual office environments.

### Escape Industries LLC - Fully Remote

**Designer, Concept Artist**  
Dates: May 2020 - Aug 2020  
- Managed a remote fabrication technician team.
- Produced concept and production visuals for escape room props, puzzles, and set construction.
- Produced concept sheets and orthographic-style production drawings for props and set elements to support fabrication and installation teams.
- Created digitally illustrated graphics for game components.
- Supported full stack from concept design through fabrication and on-site installation.
- Designed for build feasibility, schedule, and material constraints while preserving show aesthetic and interaction goals.

### Bluebird Graphic Solutions - Woburn, MA

**Sign Shop Fabricator, Graphic Design & Production**  
Dates: July 2019 - Feb 2020  
- Operated CNC equipment, laser cutters, band saws, table saws, Mimaki flatbed digital printers, large-format printers, 3D printers, and vinyl cutters in a high-output sign production environment.
- Produced, printed, and applied graphics to signage substrates across multiple materials and scales, from handheld pieces to architectural installations.
- Supported end-to-end production workflow across design, machining, painting, welding, and assembly teams while maintaining shop safety, quality standards, and delivery timelines.

## Education

### Bachelor of Fine Arts

- Institution: Rhode Island School of Design (RISD)
- Dates: 2013 - 2018
- Major: Sculpture, Interior Architecture
- Concentration: Nature-Culture Sustainability

### European Honors Program (Rome, Italy)

- Focus: Study abroad in modern sculpture and architecture
- Year: 2016 - 2017

### Fire Walk with Us (Menorca, Spain)

- Mentors: Miles Johnston, Ryan Heshka, Martin Wittfooth
- Year: 2023

## Skills

- Adobe Creative Suite (Photoshop, Illustrator, InDesign), Firefly, Procreate, Aseprite, Pixaki
- 3D modeling and technical design in Autodesk Fusion 360 and AutoCAD
- Woodworking and welding for sculpture, props, and set fabrication
- Physical modelmaking, maquette development, mold making, casting, and armature integration
- 3D printing workflows from concept to production-ready parts
- Circuit board (PCB) design in KiCad
- Programming lights, sound, sensors, and interactive effects on electronic circuits
- Integrating technology with sculpture to create pieces that move, light up, and react to people
- Production file preparation for CNC machining and laser cutting
- Scale drafting, orthographic views, and fabrication drawing standards
- Collaboration and communication across Art, Production, and Engineering teams
- Timeline management, project planning, and delivery under deadlines
- Workflow and documentation tools: Notion, Figma, Google Drive, Sheets, Docs, Slides, GitHub, Linear
